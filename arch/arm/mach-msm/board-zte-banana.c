...
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_3K_NEW
...
#define HS_NOTIFY_VBUS /*notify otg vbus state through rpc_handset,wangzy 20111227*/
extern void msm_vbus_notify_by_hs_init(void (*callback)(int online), int init);

static int msm_hsusb_vbus_hs_notif_init(void (*callback)(int online))
{
	printk(KERN_ERR "usb msm_hsusb_pmic_notif_init\n");
	msm_vbus_notify_by_hs_init(callback, 1);
	return 0;
}

static int msm_hsusb_vbus_hs_notif_deinit(void (*callback)(int online))
{
	printk(KERN_ERR "usb msm_hsusb_pmic_notif_deinit\n");
	msm_vbus_notify_by_hs_init(callback, 0);
	return 0;
}
...
static struct msm_otg_platform_data msm_otg_pdata = {
	...
	/*notify otg vbus state through rpc_handset,wangzy 20111227*/
	#ifndef HS_NOTIFY_VBUS
	.pmic_register_vbus_sn   = msm_pm_app_register_vbus_sn,
	.pmic_unregister_vbus_sn = msm_pm_app_unregister_vbus_sn,
	#else
	.pmic_register_vbus_sn   = msm_hsusb_vbus_hs_notif_init,
	.pmic_unregister_vbus_sn = msm_hsusb_vbus_hs_notif_deinit,
	#endif
	/*end*/
...
static void lcdc_lead_gpio_init(void)
{
	...
	if (gpio_request(GPIO_LCD_BL_SC_OUT, "gpio_backlight"))
...
static uint32_t lcdc_gpio_table[] = {
	...
	GPIO_CFG(GPIO_LCD_BL_SC_OUT,  0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA),
...
static unsigned bt_config_power_off[] = {
	GPIO_CFG(90, 0, GPIO_INPUT, GPIO_PULL_DOWN, GPIO_2MA),	/* WAKE */ // 42 to 90
...
//add the modemctl device resource
...
// add the GPIO config of modemctl
...
static struct i2c_board_info i2c_devices[] = {
    ...
    /*
     * add for MT9D115-2.0Mp-FF-Socket
     *
     * Attention: I2C device is initialized in sensor's driver if "CONFIG_SENSOR_ADAPTER"
     *            is defined
     */
 ...
 */
 /*
  * removed the iovdd setting to ARM9,
  */
/*
  * the board is the same as r750+,
  */
