...
static gsensor_type gsensor_chip_type=NO_CHIP_MACTH;
static struct proc_dir_entry * gsensor_entry;

//lukai add gsensor proc interface  2011-06-22

static int kernel_gsensor_read_proc(
        char *page, char **start, off_t off, int count, int *eof, void *data)
{
	int len ;

	if(1==gsensor_chip_type)
		len=sprintf(page,"%s\n","1");
	else if (2==gsensor_chip_type)
		len=sprintf(page,"%s\n","2");
	else if (3==gsensor_chip_type)
		len=sprintf(page,"%s\n","3");
	else if (4==gsensor_chip_type)
		len=sprintf(page,"%s\n","4");
...
static void init_kernel_gsensor_proc(void)
{
	gsensor_entry = create_proc_entry("gsensor_id",
				    0, NULL);
        if (gsensor_entry) {
		gsensor_entry->read_proc = kernel_gsensor_read_proc;
		gsensor_entry->data = NULL;
        }

}

static void deinit_kernel_gsensor_proc(void)
{
	if (NULL != gsensor_entry) {
		remove_proc_entry("gsensor_id", NULL);
		gsensor_entry = NULL;
	}
}
...
#if defined(CONFIG_MACH_MOONCAKE)
		if(ID_ADXL345==chipid)
			gsensor_chip_type=ADXL345;
		if(ID_ADXL346==chipid)
			gsensor_chip_type=ADXL346;
#endif
		device_flag = 1;
...
#if defined(CONFIG_MACH_MOONCAKE)
				gsensor_chip_type=LIS302DL;
#endif
				//goto probe_failed;
			}else{
#if defined(CONFIG_MACH_MOONCAKE)
				gsensor_chip_type=LIS33DE;
#endif
