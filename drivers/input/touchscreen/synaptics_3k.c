...
static const char ts_keys_size[] = "0x01:102:35:510:70:40:0x01:139:160:510:70:40:0x01:158:285:510:70:40";//ZTE_TS_XYM_20110720
#else
...
static void synaptics_rmi4_work_func(struct work_struct *work)
{
	...
	*/
	...
	if ( ts->f11.flag && (interrupt & ts->f11.intr_mask) ) 
	{
		...
		if (will_report) {
			for_each_bit(i, pointers_to_report, NR_FINGERS)
			{
				input_report_abs(ts->input_dev, ABS_MT_TRACKING_ID, i);
				input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, new_report_data[i].z);
				input_report_abs(ts->input_dev, ABS_MT_WIDTH_MAJOR, new_report_data[i].w);
...
//#if defined (CONFIG_TOUCHSCREEN_VIRTUAL_KEYS)
	...
	max_y= 2739-6;
#elif defined(CONFIG_MACH_BANANA)
	max_y=1602-138;//138 is virtualkeys y size, ZTE_TS_XYM_20110720
