...
2011-05-11  yaosujuan    fix the rtp/udp package droped    ZTE_RIL_YSJ_20110511
=======================================================*/
...
#define RMNET_DEFAULT_MTU_LEN 1500 //ZTE_RIL_YSJ_20110511
...
static void smd_net_data_handler(unsigned long arg)
{
	...
	for (;;) {
		...
		//ZTE_RIL_YSJ_20110511 start
#ifdef CONFIG_ZTE_PLATFORM
		max_package_size = RMNET_IS_MODE_IP(opmode) ?((dev->mtu > RMNET_DEFAULT_MTU_LEN)? dev->mtu:RMNET_DEFAULT_MTU_LEN) :(((dev->mtu > RMNET_DEFAULT_MTU_LEN)? dev->mtu:RMNET_DEFAULT_MTU_LEN) + ETHERNET_HEAD_LEN); 
#else
		max_package_size = RMNET_IS_MODE_IP(opmode) ? dev->mtu : dev->mtu + ETHERNET_HEAD_LEN;
#endif

//ZTE_RIL_YSJ_20110511 end
