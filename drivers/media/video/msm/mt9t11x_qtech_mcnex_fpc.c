...
/*-----------------------------------------------------------------------------------------
  ...
  2011-05-30   lijing       Fix bug of P725A high current after AF   ZTE_CAM_LJ_20110530
...
static int mt9t11x_sensor_probe_init(const struct msm_camera_sensor_info *data)
{
    ...
    if (rc < 0)
    {
        pr_err("%s: mt9t11x_sensor_dev_probe failed!\n", __func__);
        goto probe_init_fail;
    }
    ...
    if (rc < 0)
    {
        pr_err("%s: mt9t11x_reg_init failed!\n", __func__);
        goto probe_init_fail;
    }
#endif
...
static int mt9t11x_sensor_init(const struct msm_camera_sensor_info *data)
{
    ...
    {
        pr_err("%s: data is NULL, or sensor_name is not equal to mt9t11x!\n", __func__);
        rc = -ENODEV;
...
static int mt9t11x_sensor_release_internal(void)
{
    ...
    /* ZTE_ZT_CAM_20101122 ZTE_ZT_CAM_20110119 ZTE_CAM_LJ_20110530
...
#if defined(CONFIG_MACH_V9) || defined(CONFIG_MACH_ROAMER) || defined(CONFIG_MACH_TURIES)
    rc = mt9t11x_i2c_write(mt9t11x_client->addr, 0x098E, 0x3003, WORD_LEN);
    if (rc < 0)
    {
        return rc;
                                ...
                                struct msm_sensor_ctrl *s)
{
    ...
    if (rc < 0)
    {
        pr_err("%s: mt9t11x_sensor_probe_init failed!\n", __func__);
        goto probe_failed;
    }
    ...
    if (rc < 0)
    {
        pr_err("%s: mt9t11x_sensor_release failed!\n", __func__);
        goto probe_failed;
    }
#endif /* define MT9T11X_SENSOR_PROBE_INIT */
 ...
 */
    ...
    if (rc < 0)
    {
    	pr_err("%s: msm_camera_drv_start failed!\n", __func__);
        goto probe_failed;
    }
