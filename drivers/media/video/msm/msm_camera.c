...
/*-----------------------------------------------------------------------------------------
  ...
  --------     ----        -------------------------------------    ----------------------
  2011-09-29   lijing      add null pointer judgement               ZTE_CAM_LJ_20110929
                                 ...
                                 int (*sensor_dev_probe)(const struct msm_camera_sensor_info *))
{
    ...
    if(0 == sensor_init_status)
    {
        ...
        if (rc < 0)
        {
            pr_err("%s: i2c_dev_probe_on failed!\n", __func__);
        ...
        if (rc < 0)
        {
            pr_err("%s: msm_camio_probe_on failed!\n", __func__);
			rc = -ENOINIT;
            goto dev_start_exit;
        }
        ...
        if (rc < 0)
        {
            pr_err("%s: sensor_dev_probe failed!\n", __func__);
        ...
        if (rc < 0) 
        {
            pr_err("%s: msm_camio_probe_off failed!\n", __func__);
            goto dev_start_exit;
        }
    }
    else
    {
        rc = -ENOINIT;
        pr_err("%s: entry, no need to initialize name=%s \n", __func__,dev->name);
    }
